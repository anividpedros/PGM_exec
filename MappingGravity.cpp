...
{  
	...
	// READ GRAVITY FILE
	// std::ifstream GravityFile("GravityFile_hybrid_2r.txt");
	std::ifstream GravityFile("/Users/anivid/Documents/Research/HybridShapeModel/Code/GravityFiles/truth.txt");
    Polygon polygon(GravityFile);
	Vect grav_output(12); // Acceleration + gravity matrix + position


	// READ FILE WITH MAPPING POINTS
	std::ifstream MapGravity("GravityMapping/Points_Itokawa.txt");
	...
	if (world_rank == 0)
	{	
		std::ofstream GravityFile ("GravityMapping/GravityMap_2r.txt");
		...
		std::ofstream GravityGradientFile ("GravityMapping/GravityGradient_2r.txt");
